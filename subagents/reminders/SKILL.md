...
**Phase 0 — Orient.** Determine "today" in the owner's configured timezone (after-midnight counts as the
prior day) and which run this is: the daily **seed** (the once-per-day whole-day enqueue + reset), the
**Wrap/Dream** journal-presence check, or an **interactive** ask ("remind me to…", "what's still open").
**If the Reminders domain isn't provisioned yet, say so and stop** — there's nothing to track until it
exists (see `databases.md` / `store/<backend>/schema.md`).
...
- **Journal-presence-gated habits (auto-satisfy from journal activity).** For any Recurring Habit whose
  ...
  it in the **Wrap** run (nudge-or-satisfy) + **Dream** (satisfy-only backstop). See
  `reminders-policy.md` → "Journal-presence gate."
...
- **Compute the day's fires (seed):** every `pending` row due today, at its `times` (or the `time_window`
  default). Rows that re-fire (`importance: super-critical`/`critical`/`high` **OR**
  `nag_until_done: true`) also get the **90-min re-fire ladder** through end of day —
  `reminders_seed.py` computes both per row (Phase 3). `snoozed` items re-surface `+30 min` out.
...
- **Seed run — enqueue the whole day per row via `reminders_seed.py`.** For each due Reminders row, one
  ...
  `time_window` default) **plus** the 90-min re-fire ladder for importance ≥ high / `nag_until_done`:
  ...
  [--pierce-quiet]`. It writes `../../seneschal/state/reminders.json` idempotently (stable ids
  `rmd-<date>-<slug>-<HHMM>`); the resident `presence.py` daemon fires each at its due minute and stamps
  `fired_at` (**Dream** prunes fired entries). Add `--pierce-quiet` for critical-and-above rows. End each
  `--text` with the v1 ack hint: *"tick Ack or tell me."*
...
- **`Call Me` items → also ring the owner.** For any fired reminder with `call_me: true`, enqueue a
  **second**, call-channel entry a few minutes out, phrased for the ear (no ⏰ prefix — it's spoken):
  ...
  --channel call --id rmd-<YYYY-MM-DD>-<slug>-call --due-at <due+~3min, UTC>`. Rare + opt-in; the daemon
