...
| `breakglass-assertion-secret` | cockpit backend / `cockpit/breakglass/assertion.py` (first use) | Auto-generated **break-glass assertion signing secret** (random 32 bytes) — DELIBERATELY separate from `cockpit-session-secret`, so a stolen session cookie can never mint a valid break-glass assertion. Shared by the minter (the backend's `bg`-flagged `/auth/callback`) and the verifier (the supervisor). Delete to rotate. |
