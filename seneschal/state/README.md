...
| `telegram-message-map.json` | `sentinel.py` (`record_sent_message`, on the nudge fire path) | Bounded map `message_id → {kind, text, reminder_id?, sent_at}` of the assistant's outbound Telegram messages (newest 200), so a future inbound **reaction** (a 👍 on a nudge) can be tied back to what it reacted to. Best-effort context, fail-open; groundwork — nothing reads it yet. |
